...
getdoc <- function(cushion="localhost", dbname, docid, as='list', ...)
{
  cushion <- get_cushion(cushion)
  if(cushion$type=="localhost"){
    sofa_GET(sprintf("http://127.0.0.1:%s/%s/%s", cushion$port, dbname, docid), NULL, as, ...)
  } else if(cushion$type %in% c("cloudant",'iriscouch')){
    sofa_GET(file.path(remote_url(cushion, dbname), docid), NULL, as, content_type_json(), ...)
  }
}
