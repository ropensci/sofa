...
#' }
#' @export
sofa_view_put <- function(dbname, design_name, endpoint="http://127.0.0.1", port=5984)
{ 
  call_ <- paste0(paste(endpoint, port, sep=":"), "/", dbname, "/_design/", design_name)
  doc2 <- paste0('{"_id":', '"_design/', design_name, '",', '"views": {', '"foo": {', '"map": "function(doc){emit(null, doc)}"}}}')
  PUT(url=call_, body=doc2) 
}  
...
sofa_view_search <- function(dbname, design_name, query = NULL, endpoint="http://127.0.0.1", port=5984)
{
  ...
  args <- list(...)
  fromJSON(content(GET(url=call_, query=args)))
}